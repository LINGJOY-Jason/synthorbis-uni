...
typedef enum SanctifyPanicLevel {
  SANCTIFY_PANIC_ASSERT,  // 断言失败（不应该发生的逻辑错误）
...
// ─────────────────────────────────────────────────────────────
//  Panic 回调（允许自定义处理，如写 minidump / 弹窗 / 重启）
// ─────────────────────────────────────────────────────────────
...
/**
 * 设置自定义 panic 回调（默认实现为 fprintf + abort）
 */
 ...
 * 触发 fatal panic（永不返回）
 */
SANCTIFY_API __attribute__((noreturn)) void
...
#if defined(_WIN32)
  #define SANCTIFY_ABORT() do { \
    __debugbreak(); \
    abort(); \
  } while (0)
#else
  #define SANCTIFY_ABORT() abort()
#endif
